...
def run(beta,gamma,save):
    command = f"horovodrun -np 4 -H localhost:4 python {file}.py --num-workers 8 --enco-modules 2 --enco-res --train-manifest {train_} --val-manifest {test_} --cuda --rnn-type gru --hidden-layers 5 --hidden-size 1024 --epochs 50 --lr 0.001 --batch-size {batch} --checkpoint --gpu-rank {gpu_rank} --update-rule 2 --exp-name save/{save} --mw-alpha 0.0001 --mw-beta {beta} --mw-gamma {gamma} --forg-modules 2 --forg-res --num-epochs 10 --patience 10" 
